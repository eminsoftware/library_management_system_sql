...
-- after creating all tables, click on database on left sidebar and then ERD for Database

-- foreign keys
-- after adding FKs, refresh the database and create the new ERD
...
-- while importing in books table, varchar(10) was short so set new data type
alter table books
...
-- CROSS JOIN will just add that one value (example_date) to every row of your result
...
-- if used, create or replace deletes if such procedure already exists
-- p. (parameter) is a naming convention, means that the input is expected from user
...
/* Task 15. For each branch_id, show the number of books issued, 
